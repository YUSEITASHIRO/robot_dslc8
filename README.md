...
| **Phase 6.1** | 会話フェーズ追跡とプロンプト動的更新 | 🔲 計画中 |
...
main（全 Phase 統合）
  └─ feat/phase1-navigation   ← Phase 1/2/3 実装
       └─ feat/phase4-quality ← Phase 4 完了
            └─ feat/phase5-enhancement ← Phase 5 完了（現在の最新開発ブランチ）
                 └─ feat/phase6-xxx ← Phase 6 予定
```
...
| `feat/phase5-enhancement` | Phase 5 エコーキャンセル + シーンキャッシュ | ✅ 実装済み（PR 待ち） |
...
├── src/
│   ├── dialogue_system/
│   │   ├── g1_realtime_dialogue.py  ★ メインエントリポイント（Phase 1〜5 実装済み）
