...
main（全 Phase 統合・保護済み）
  ...
  ├─ feat/phase5-enhancement  ← Phase 5 完了（マージ済み）
  ├─ feat/phase6-strategy     ← Phase 6.1 + 7.1/7.2（マージ済み）
  ├─ feat/phase6.2-webrtc     ← Phase 6.2 WebRTC 仮実装（マージ済み）
  └─ fix/webrtc-speaker-track ← バグ修正: _SpeakerTrack + get_running_loop（PR 作成済み）
```
...
| `feat/phase5-enhancement` | Phase 5 エコーキャンセル + シーンキャッシュ | ✅ マージ済み |
...
| `fix/webrtc-speaker-track` | _SpeakerTrack 動的継承廃止 + asyncio API 統一 | 🔲 PR レビュー中 |
