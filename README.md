...
```
main（全 Phase 統合）
  └─ feat/phase1-navigation   ← Phase 1/2/3 実装
       └─ feat/phase4-quality ← Phase 4 完了
            └─ feat/phase5-enhancement ← Phase 5 完了
                 └─ feat/phase6-strategy ← Phase 6.1 + Phase 7.1/7.2 完了
                      └─ feat/phase6.2-webrtc ← Phase 6.2 WebRTC 仮実装（現在の最新開発ブランチ）
```

| ブランチ | 内容 | 状態 |
|---------|------|------|
| `main` | 全 Phase 統合済み（PR マージ経由） | ✅ 安定版 |
| `feat/phase1-navigation` | Phase 1/2/3 全実装 + ペルソナ設計 | ✅ マージ済み |
| `feat/phase4-quality` | Phase 4 品質・安定性改善 + 運営版同期 | ✅ マージ済み |
| `feat/phase5-enhancement` | Phase 5 エコーキャンセル + シーンキャッシュ | ✅ マージ済み |
| `feat/phase6-strategy` | Phase 6.1 フェーズ追跡 + Phase 7.1/7.2 Vision 監視 | ✅ 実装済み（PR 待ち） |
| `feat/phase6.2-webrtc` | Phase 6.2 AudioBackend 抽象化 + WebRTCAudioBackend 仮実装 | ✅ 実装済み（PR 待ち） |
