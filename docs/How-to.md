...
- Phase 5.1 で音声ゲート方式として実装済み（下記参照）

#### 4.2 転倒防止・フェールセーフ

- `navigate_to` に NAV_TIMEOUT_S（60秒）の watchdog を実装
- タイムアウト時は `walker.stop()` で強制停止

#### 4.3 レイテンシ最適化

- VAD パラメータ（threshold / prefix_padding_ms / silence_duration_ms）を定数管理
- TURN_REPEAT_COUNT / TURN_REPEAT_INTERVAL を運営推奨値に設定

---

### Phase 5: 音声品質・Vision 高速化 ✅ 完了

#### 5.1 エコーキャンセル（音声ゲート方式）

`ECHO_GATE_COOLDOWN_S = 0.40` 秒のゲートタイムで TTS 再生中のマイク入力を遮断し、自己エコーによる誤検出を防止する。

- `play_audio()` がチャンクを書き込むたびに `_speaking_until` を更新
- `stream_mic()` はゲート期間中のマイク入力をドロップ
- `speech_started` イベント受信時にゲート中なら応答せずゲートを延長

#### 5.2 シーンキャッシュ + 初回自動スキャン

Vision API の呼び出しを減らしつつ初動のレイテンシを下げる。

- `_analyze_scene()` は TTL 30秒のキャッシュを参照し、有効期間内は API をスキップ
- `connect()` 完了後 3秒で `_initial_scene_scan()` を非同期実行してキャッシュを温める
- `look_around` 呼び出し時にキャッシュが warm な場合は即時返却
