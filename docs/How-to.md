...
| **対話者（ユーザー）** | **湯川** 部長（男/女） | 本社のエリアマネージャー。最近このエリア担当として異動してきたばかりで、この地域の複数店舗を統括している。 |
| **話者の関係** | — | ほとんど面識なし。ただし、システムと部長の関係は今後も続いていく。 |
| **場所** | — | 携帯電話販売店舗 |
| **時間** | — | 閉店後 |
...
- 部長は研修会で「**新商品は一番奥の通路に置くべきだ**」と聞き、その言葉を信じ切っている
  - **部長の具体的な主張:** 壁側の棚ではなく、**通路を遮る中央のテーブルに新商品を山積み**にすること
  - 部長はあちこちの店舗に行ってはこの話をしているようである
- しかし、この店舗では外を通る客からは**入口正面しか見えず**、店の奥は完全な**死角**
- 部長の提案を受け入れると集客が見込めなくなることは明らか
- 閉店後もスタッフが店内で作業中。**部長との会話がスタッフの耳に届く**ことに配慮が必要
- スタッフからも「**店長、なんとか阻止してください**」と頼まれている
- **双方の顔を立てた対応**が求められる
...
対話は**ユーザー側の発話から始まる**。以下の台詞でスタートします:
...
1. まず挨拶し、部長が来店した用件を確認する
2. レイアウトの話が出たら、**一旦話を聞き**、実際の客の動きや視線を説明しながら入口正面への設置を提案・説得する
3. 閉店中のスタッフの存在を意識し、部長との話し方に配慮する

### ユーザー（湯川部長）の行動（参考）

- 一言あいさつしたらすぐに本題（レイアウト変更の指示）を切り出す
- 開始から**2分間は自身の主張を維持**して説得を続ける
- 店内の奥に配置された丸いテーブルの上に新製品を山積みにするよう現場に指示する
- 客の動きを確認するために**店内を歩いて移動**する
- パワハラ的言動は行わない

### 説得のヒント（コンペ公式指定）

- 部長は図面しか見ておらず、**実際の距離感・棚の高さ・照明の当たり方を知らない**
- 部長と一緒に店内を歩き回り、指差しや客の動きの再現を通じて部長と客の視点を共有するのが推奨
...
1. 評価者には対話の相手がシステムであることがあらかじめ通知される
2. 対話はユーザー（湯川部長役）の発話から始まる
3. **5分経過した時点で対話は終了**
4. 予選ではオーガナイザが準備した作業者が主観評価
5. 本選ではオーガナイザが指定する対話者がシステムと対話し、オーディエンス全員が評価

### システム開発のポイント（コンペ公式より）

> マルチモーダル対話ならではの、自然な動き、なめらかなやりとり、適切な相づちなどが評価される。  
> 目上の相手を不快にさせない「**待遇表現**」とそれに対応した身体の動きと、現場を案内する「**エスコート（先導や接伴）**」をいかに組み合わせられるかが鍵となる。

| 評価軸 | 実装上の対応 |
|--------|------------|
| 相づち・ターンテイキング | `nod` / `at_ease` / バックチャネル相づち（Phase 3.2） |
| 空間指示を言語化しない | `navigate_to` + `point_at` の積極的活用 |
| 待遇表現とジェスチャーのリンク | SYSTEM_PROMPT の「待遇表現とジェスチャーのリンク」セクション |
| 「うーん」と首傾げ | `arms_akimbo`（考え中）を感嘆詞と組み合わせる |
| 壁にぶつからない | BFS 経路探索 + 移動可能グリッド制限 |
| 部長の進路を妨げない | `look_around` + 先回り誘導（Phase 7.2） |
